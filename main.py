...
def CalculateNorm(arrayAnalitical, array2):
    ...
    amplitude = np.amax(normArray) - np.amin(normArray)
    indexMax = np.argmax(normArray)
    '''
    return 100*RRHS_error
...
ax1.grid('on')
ax1.plot((dtArray[1:]), (error[1:]), 'b-o', label='Simplified Backward Euler', linewidth=4, markersize = 10)
ax1.plot((dtArray[1:]), (error_e[1:]), 'g-o', label='Forward Euler', linewidth=4, markersize = 10)
#ax1.set_ylim([0, 50])
extraticks = [5]
